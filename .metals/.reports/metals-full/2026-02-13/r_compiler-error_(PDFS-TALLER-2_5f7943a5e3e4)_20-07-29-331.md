...
def enriqueser(transacciones: List[Transaccion]) = {
  transacciones.map{ t =>
    ...
    t match {
      ...
      //Cuando es CREDITO
      case Transaccion(_, productoI@@, monto, _, "CREDITO", _, _) =>

        //Buscar el producto comprado en la transaccion
        //.find() retorna un Option[Producto]
        val producto = BancoData.productos.find(_.idProducto == producto)
        ...
        // Si es None -> 0.0
        val tasaInteres = producto.map(_.tasaInteres).getOrElse(0.0)
